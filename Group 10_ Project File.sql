DROP DATABASE IF EXISTS Nutrition_System_Team_10;
CREATE DATABASE IF NOT EXISTS Nutrition_System_Team_10;
USE Nutrition_System_Team_10;
...
-- 2. Retrieving the user details along with their addresses
SELECT u.user_id_pk,u.ufname,u.ulname,u.contact,a.address_line1,a.city,a.state,a.zip FROM users_t1 u 
JOIN addresses_t10 a ON u.user_id_pk=a.user_id_fk;
...
-- 7. Provide an overview of users and the count of content items they have viewed.
...
SELECT User_id_fk, MAX(Workout_duration) AS Longest_Workout, MIN(Workout_duration) AS Shortest_Workout, STDDEV(Workout_duration) 
AS Duration_Variability FROM Progress_Log_T4 GROUP BY User_id_fk;
...
END; //
DELIMITER ;
call insert_fitness_goals(9,"Deadlift","2023-12-30");
select * from fitnessgoals_audit_t16;

 
-- creating a trigger for deleted trainer details and to store the deleted trainer data to a trainer audit table
